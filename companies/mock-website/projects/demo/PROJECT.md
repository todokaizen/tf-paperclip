...
Demo project used by the TF-Devflow token-efficiency benchmark. The workspace at `/tmp/mock-website-current/` is re-staged from `fixture-repo/` before every run, so every benchmark starts from byte-identical source.
